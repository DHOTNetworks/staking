...
function App() {
  ...
  const [Metra, setMetra] = useState(0);
  ...
  function getContractAddress() {
    return "0x2c46d80ee468472f8c98fe358f6ccb1c378eadea";
  }
  ...
  async function connectToSelectedNetwork() {
    ...
    const metra = await new web3.Contract(abi, address);
  ...
  async function getUserProfile() {
    ...
    call(Metra.methods.totalSupply, setTotalSupply);
    // balanceOf Requires input argument of the account to grab, so let's grab the first available account for now
    call(Metra.methods.balanceOf, setAccountBalance, accounts[0]);
    call(Metra.methods.hasStake, setAccountStakes, accounts[0]);
  ...
  function stake() {
    ...
    Metra.methods.stake(1000).estimateGas({from: accounts[0]})
      .then((gas) => {
        // We now have the gas amount, we can now send the transactions
        Metra.methods.stake(1000).send({
          from: accounts[0],
          gas: gas
        });
